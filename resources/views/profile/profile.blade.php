            ...
            <div class="dis-flex flex-col flex-m bor12 p-t-15 p-b-30">
                <div class="size-210 p-r-18 p-r-0-sm w-full-ssm">
                    ...
                    <div class="p-t-15">
                        ...
                        <form action="{{ route('updateAddress') }}" method="POST" class="m-b-22">
                            @csrf
                            @method('PUT')
                            <div class="bg0 m-b-22">
                                <x-input type="text" name="country" placeholder="Enter your country" value="{{ old('country' , $user->country) }}" /> 
                                @error('country')
                                <x-error>{{ $message }}</x-error>
                                @enderror
                            </div>
                            <div class="bg0 m-b-22">
                                <x-input type="text" name="city" placeholder="Enter your city" value="{{ old('city' , $user->city) }}" />
                                @error('city')
                                <x-error>{{ $message }}</x-error>
                                @enderror
                            </div>
                            <div class="bg0 m-b-22">
                                ...
                                <x-input type="text" name="street" placeholder="Enter your street" value="{{ old('street' , $user->street) }}"  />
                                @error('street')
                                <x-error>{{ $message }}</x-error>
                                @enderror
                            </div>
